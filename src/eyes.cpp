...
void RoboEyes::begin(int w, int h, int fps){
  ...
  // defaults based on chosen size (scale from 128x64 reference)
...
void RoboEyes::setMood(Mood m){ tired_=angry_=happy_=false; if(m==TIRED) tired_=true; else if(m==ANGRY) angry_=true; else if(m==HAPPY) happy_=true; }
...
void RoboEyes::drawEyes(){
  ...
  if(hFlicker_){ int off = hFlickerAlt_? hFlickAmp_ : -hFlickAmp_; eyeLx_ += off; eyeRx_ += off; hFlickerAlt_ = !hFlickerAlt_; }
